...
test('fetches static data', async t => {
	...
	const scope = nock('http://localhost').get('/worldconquest/maps/map-name/static').reply(200, apiBody);

	const client = new Client('http://localhost');

	const staticData = await client.map.fetchStaticData('map-name');
